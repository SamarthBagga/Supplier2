...
// OrderItem component with translation
...
// OrderCard component with OrderItem integration
    ...
    const fetchActiveOrders = useCallback(async () => {
        try {
            ...
            const response = await axios.post(
                'https://mandie.co.in/api/supplier/active-orders',
                { phoneNumber }
            );
            setActiveOrders(response.data.activeOrders || []);
            console.log('Active orders:', response.data.activeOrders);
        } catch (error) {
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.content}>
                <Text style={styles.title}>{i18n.t('activeOrders')}</Text>
